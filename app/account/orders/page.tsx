  ...
  return (
    <div className="max-w-[900px] mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold">Історія покупок</h1>
      </div>

      {ordersLoading ? (
        <div className="text-center py-12 text-(--muted)">Завантаження...</div>
      ) : orders.length === 0 ? (
        <div className="text-center py-16">
          <div className="text-5xl mb-4">📦</div>
          <p className="font-semibold mb-1">У вас ще немає замовлень</p>
          <p className="text-(--muted) text-sm mb-6">Тут з&apos;являться ваші покупки після оформлення замовлення</p>
                ...
                {order.items.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-(--muted)">{item.qty} шт. × {item.price.toLocaleString('uk-UA')} ₴</div>
                    </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-3 border-t border-(--border) text-sm">
                <div className="text-(--muted)">
                  ...
                  {order.trackingNumber && (
                    <span className="ml-3">
                      ТТН: <span className="font-mono font-semibold text-(--text)">{order.trackingNumber}</span>
                    </span>
                  )}
                </div>
